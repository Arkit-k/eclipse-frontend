...
export const authApi = apiSlice.injectEndpoints({
  ...
  endpoints: (builder) => ({
    ...
    saveOrder: builder.mutation({
      query: (data) => ({
        url: "https://eclipse-backend.vercel.app/api/order/saveOrder",
        method: "POST",
        body: data,
      }),
    ...
    getUserOrders: builder.query({
      query: () => `https://eclipse-backend.vercel.app/api/user-order`,
      providesTags:["UserOrders"],
      keepUnusedDataFor: 600,
    }),
    // getUserOrders
    getUserOrderById: builder.query({
      query: (id) => `https://eclipse-backend.vercel.app/api/user-order/${id}`,
